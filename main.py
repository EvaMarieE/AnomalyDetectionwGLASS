...
@click.argument("mask_path", type=click.Path(exists=True, file_okay=False))
...
def dataset(
    ...
    _DATASETS = {"mvtec": ["datasets.mvtec", "MVTecDataset"], "visa": ["datasets.visa", "VisADataset"],
                 ...
                 "chest" : ["datasets.chest", "ChestXrayDataset"] }
    ...
    def get_dataloaders(seed, test, get_name=name):
        ...
        train_dir = os.path.join(data_path, "train", "1")  # hardcoded training subset folder
        test_image_dir = os.path.join(data_path, "test_anomaly", "images")  # test images folder
        test_lung_mask_dir = os.path.join(data_path, "test_anomaly", "lung_masks")  # test masks folder
        anomaly_mask_dir = os.path.join(data_path, "test_anomaly", "anomaly") # mask for anomaly / ground truth

        # Filter test images by existing anomaly masks
        test_image_paths = sorted([
            os.path.join(test_image_dir, fname)
            for fname in os.listdir(test_image_dir)
            if os.path.isfile(os.path.join(anomaly_mask_dir, fname)) 
        ])
        ...
        test_dataset = dataset_library.__dict__[dataset_info[1]](
            data_path,
            aug_path,
            mask_path=anomaly_mask_dir,
            train_dir=train_dir,
            test_image_dir=test_image_dir,
            test_lung_mask_dir=test_lung_mask_dir,
            resize=resize,
            imagesize=imagesize,
        ...
        test_dataloader.name = get_name + "_wAnomalyMask"
        ...
        if test == 'ckpt':
            ...
            LOGGER.info(f"Dataset {'1':^20}: train={len(train_dataset)} test={len(test_dataset)}")
        else:
            train_dataloader = test_dataloader
            LOGGER.info(f"Dataset {'1':^20}: train={0} test={len(test_dataset)}")
...
def run(
    ...
    writer = SummaryWriter(log_dir=os.path.join(run_save_path, 'tensorboard'))
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        glass_list = methods["get_glass"](input_shape, device)
        ...
        for i, GLASS in enumerate(glass_list):
            ...
            if test == 'ckpt':
                flag = GLASS.trainer(dataloaders["training"], dataloaders["testing"], dataset_name, writer=writer)
                if type(flag) == int:
...
if __name__ == "__main__":
    ...
    tb.configure(argv=[None, '--logdir', 'results/glass_project/run_01/tensorboard'])
    url = tb.launch()
